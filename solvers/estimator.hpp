#ifndef OPEN_SFM_METU_SOLVERS_ESTIMATOR_HPP
#define OPEN_SFM_METU_SOLVERS_ESTIMATOR_HPP
...
template <typename DatumType, typename ModelType> class Estimator {
	...
	public:
		...
		// estimation. Typically, this is a minimal set, but it is not required to be.
		virtual bool EstimateModel(const std::vector<Datum>& data,
		                         std::vector<Model>* model, std::vector<double>* l_values, int imageWidth, int point_numCorr) const = 0;
...
#endif // OPEN_SFM_METU_SOLVERS_ESTIMATOR_HPP